...
function isLogEntry(obj: unknown): obj is LogEntry {
    ...
    return (
        typeof record.time_unix === "number" &&
...
export async function getLogFiles(dateRange?: { start?: string; end?: string }): Promise<string[]> {
    const files: string[] = [];
...
export function applyFilters(entries: LogEntry[], filter: LogFilter): LogEntry[] {
